...
from source_provenance import canonical_bundle_sha256, sha256_file
...
def fetch_registry_links(registry_url: str) -> list[tuple[str, str]]:
    with urllib.request.urlopen(request(registry_url), timeout=60) as response:
        validate_https_url(response.geturl(), "technicalSource.registryUrl")
        ...
        parser.feed(text)
        return [(urljoin(response.geturl(), href), html.unescape(label)) for href, label in parser.links]
...
def resolve_part_url(links: list[tuple[str, str]], expected_filename: str) -> str:
    ...
    for href, label in links:
        if normalized_filename(label) == expected or normalized_filename(href_filename(href)) == expected:
            return validate_https_url(href, f"technical part {expected_filename}")
...
def process_technical_source(entry: dict) -> None:
    ...
    registry_url = validate_https_url(entry.get("registryUrl"), "technicalSource.registryUrl")
    ...
    for index, part in enumerate(parts, start=1):
        ...
        checksum, size_bytes, final_url = download_file(url, destination, f"technicalSource part {index}")
    ...
    for key in ("verifiedBy", "verifiedAt", "verificationMarkers"):
        entry.pop(key, None)
