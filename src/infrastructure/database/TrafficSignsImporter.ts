...
function safeImagePath(value: string): string {
  ...
  if (
    ...
    throw new Error(`Unsafe traffic sign image path: ${value}`);
  }
  ...
  if (!ALLOWED_IMAGE_EXTENSIONS.has(extension)) {
    throw new Error(`Unsupported traffic sign image type: ${value}`);
  }
...
export function validateTrafficSignsDataset(dataset: TrafficSignsDataset): void {
  ...
  if (dataset.stage !== "production") {
    throw new Error(`Traffic signs dataset stage must be production, found: ${dataset.stage}`);
  }
  if (!dataset.version?.trim() || !VERSION_RE.test(dataset.version.trim())) {
    throw new Error("Traffic signs dataset version is invalid");
  }
  if (!dataset.validFrom?.trim() || !ISO_DATE_RE.test(dataset.validFrom.trim())) {
    throw new Error("Traffic signs dataset validFrom must use YYYY-MM-DD");
  }
  if (!dataset.sourceDocument?.trim()) {
    throw new Error("Traffic signs dataset sourceDocument is required");
  }
  if (!isSha256(dataset.sourceSha256)) {
    throw new Error("Traffic signs dataset sourceSha256 must be a valid source document SHA-256");
  }
  ...
  for (const sign of dataset.signs) {
    ...
    if (codes.has(code)) {
      throw new Error(`Duplicate traffic sign code: ${code}`);
    }
    codes.add(code);
    ...
    if (sign.image !== undefined && sign.image !== null && sign.image !== "") {
      if (typeof sign.image !== "string") throw new Error(`${code}: image must be a string`);
      safeImagePath(sign.image);
    }
  }
}
...
async function metadataValue(db: Database, key: string): Promise<string | null> {
  const rows = await db.select<MetadataRow[]>(
    "SELECT value FROM traffic_sign_metadata WHERE key = $1",
    [key],
  );
...
export async function getLocalTrafficSignsState(): Promise<TrafficSignsLocalState> {
  ...
  const [version, validFrom, sourceDocument, sourceSha256, contentSha256, assetSha256, count] = await Promise.all([
    ...
    metadataValue(db, "validFrom"),
    metadataValue(db, "sourceDocument"),
...
export class TrafficSignsImporter {
  async import(
    ...
    options: TrafficSignsImportOptions = {},
  ): Promise<TrafficSignsImportResult> {
    ...
    if (
      ...
      currentCount === dataset.signs.length &&
      normalizedChecksum(currentContentSha256) === contentSha256 &&
      normalizedChecksum(currentAssetSha256) === assetSha256
    ) {
